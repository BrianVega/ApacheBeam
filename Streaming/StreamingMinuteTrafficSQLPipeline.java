...
public class StreamingMinuteTrafficSQLPipeline {
    ...
    public static PipelineResult run(Options options) {
        ...
        PCollection<CommonLog> logs = 
        // Reads from the PubSub topic provided by the command line with PubsubIO.readStrings(). Remember to set the Timestamp attribute.
        pipeline.apply("ReadFromPubSub", PubsubIO.readStrings().fromTopic(options.getInputTopic()).withTimestampAttribute("timestamp"))
        // Parses the Json to a object with a ParDo transform
                .apply("JsonToCommonLog", ParDo.of(new JsonToCommonLog()));

        logs
        // Adds a DateTime field called "timestamp_joda" with AddFields.<CommonLog>create().field()
                .apply("AddTimestamp_jodaField", AddFields.<CommonLog>create().field("timestamp_joda", FieldType.DATETIME))

        // Populates that field by building a new Row. Perform this in a MapElements.via(new SimpleFunction<Row, Row>) transform
                .apply("EditSchemaAddingTSValue", MapElements.via(new SimpleFunction<Row, Row>(){
                    @Override
                ...
                // To complete this task implement a SqlTransform that windows the data each minute and counts the number of pageviews.
                .apply("PageViewsPerMinute", SqlTransform.query("SELECT COUNT(*) AS pageviews FROM TUMBLE ((SELECT * FROM PCOLLECTION)"
                + ", DESCRIPTOR(timestamp_joda), \"INTERVAL 1 MINUTE\") AS tr GROUP BY tr.window_start"))
